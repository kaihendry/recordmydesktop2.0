...
ffmpeg $verbose $duration -f x11grab -s $res -i :0.0 -vcodec huffyuv -sameq -acodec pcm_s16le -f alsa -i hw:0,0 $out
test -s $out || ( rm $out; echo ffmpeg failed, try $0 -v; exit 1 )
echo "<video controls src=$out></video>" > $out.html
if test "$dest"
then
	today=$(date --iso-8601=date)
	rsync --progress --remove-source-files $out $out.html $dest/$today/
